...
ENTRY_START = dtime(9, 45)   # avoid first 15 min chop
CUTOFF      = dtime(15, 45)  # no new entries after this

# Bull/bear pairs and their underlyings (also ingested for context)
PAIRS = {
...
def compute_indicators(df: pd.DataFrame) -> dict:
    # VWAP — primary day trading signal
    df["vwap"] = (
        (df["volume"] * (df["high"] + df["low"] + df["close"]) / 3).cumsum()
        / df["volume"].cumsum()
    )
    # Short EMAs for momentum
    df.ta.ema(length=9, append=True)
    df.ta.ema(length=21, append=True)
    df.ta.rsi(length=9, append=True)
    df.ta.macd(fast=12, slow=26, signal=9, append=True)
    df.ta.atr(length=14, append=True)  # volatility sizing

    # Opening range: first 15 bars (avoid first 15 min for entry but track range)
    or_bars = df.head(15)
    ...
    return {
        ...
        "ema_9":        round(float(last.get("EMA_9", 0)), 4),
        "ema_21":       round(float(last.get("EMA_21", 0)), 4),
        "ema9_above_21": bool(last.get("EMA_9", 0) > last.get("EMA_21", 0)),
        "rsi_9":        round(float(last.get("RSI_9", 0)), 2),
        "macd":         round(float(last.get("MACD_12_26_9", 0)), 4),
        "macd_signal":  round(float(last.get("MACDs_12_26_9", 0)), 4),
        "macd_bullish": bool(last.get("MACD_12_26_9", 0) > last.get("MACDs_12_26_9", 0)),
        "atr_14":       round(float(last.get("ATRr_14", 0)), 4),
        "volume":       int(last["volume"]),
        "volume_ratio": round(vol_ratio, 2),   # > 1.5 = above-average volume
...
def call_llm(symbol: str, indicators: dict, underlying_indicators: dict | None,
...
Underlying index ({und}) — this is your PRIMARY directional signal:
{json.dumps(underlying_indicators, indent=2)}

{symbol} is a {direction} ETF tracking {und}.
If {und} is above its VWAP and trending up → favor TQQQ/UPRO/SOXL side.
If {und} is below its VWAP and trending down → favor SQQQ/SPXU/SOXS side.
"""
...
CRITICAL RULES for leveraged ETFs:
...
4. If < 30 min left: only "sell" to exit or "hold" existing, NO new "buy" entries
5. If < 15 min left: only "sell" or "hold"
6. RSI > 75 = overbought for leveraged ETF (moves fast); RSI < 25 = oversold
{underlying_section}
{symbol} indicators (1-min bars):
{json.dumps(indicators, indent=2)}

Recent 1-min closes for {symbol} (oldest → newest):
{recent_closes}
...
def run_analysis(symbol: str):
    ...
    # Fetch underlying indicators for context
    underlying_ind = None
